...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: exclusivecoin.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: exclusivecoind.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeexclusivecoinamount=<n> " + _("Keep N ExclusiveCoin anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. ExclusiveCoin is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. ExclusiveCoin is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("ExclusiveCoin version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    nAnonymizeExclusiveCoinAmount = GetArg("-anonymizeexclusivecoinamount", 0);
    if(nAnonymizeExclusiveCoinAmount > 999999) nAnonymizeExclusiveCoinAmount = 999999;
    if(nAnonymizeExclusiveCoinAmount < 2) nAnonymizeExclusiveCoinAmount = 2;
    ...
    LogPrintf("Anonymize ExclusiveCoin Amount %d\n", nAnonymizeExclusiveCoinAmount);
