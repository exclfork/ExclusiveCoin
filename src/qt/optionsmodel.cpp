...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeExclusiveCoinAmount"))
        settings.setValue("nAnonymizeExclusiveCoinAmount", 1000);
    nAnonymizeExclusiveCoinAmount = settings.value("nAnonymizeExclusiveCoinAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeExclusiveCoinAmount"))
        SoftSetArg("-anonymizeexclusivecoinamount", settings.value("nAnonymizeExclusiveCoinAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeExclusiveCoinAmount:
            return QVariant(nAnonymizeExclusiveCoinAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeExclusiveCoinAmount:
            nAnonymizeExclusiveCoinAmount = value.toInt();
            settings.setValue("nAnonymizeExclusiveCoinAmount", nAnonymizeExclusiveCoinAmount);
            emit AnonymizeExclusiveCoinAmountChanged(nAnonymizeExclusiveCoinAmount);
            break;
        default:
            break;
        }
