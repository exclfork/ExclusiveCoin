...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeExclusiveCoinAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeExclusiveCoinAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeExclusiveCoinAmount = strAnonymizeExclusiveCoinAmount.remove(strAnonymizeExclusiveCoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeExclusiveCoinAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeExclusiveCoinAmount*COIN) nMaxToAnonymize = nAnonymizeExclusiveCoinAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeExclusiveCoinAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeExclusiveCoinAmount));
        strAnonymizeExclusiveCoinAmount = strAnonymizeExclusiveCoinAmount.remove(strAnonymizeExclusiveCoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeExclusiveCoinAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
...
void OverviewPage::toggleDarksend(){
    ...
    if(!fEnableDarksend){
        ...
        if(nAnonymizeExclusiveCoinAmount == 0){
